...
public class PacketIfaceTermUpdate extends AppEngPacket {
    ...
    private void decode(ByteBuf buf) {
        ...
        for (int i = 0; i < numEntries; ++i) {
            ...
            try {
                switch (type) {
                    case 0 -> this.commands.add(new PacketAdd(buf));
                    case 1 -> this.commands.add(new PacketRemove(buf));
                    case 2 -> this.commands.add(new PacketOverwrite(buf));
                    case 3 -> this.commands.add(new PacketRename(buf));
